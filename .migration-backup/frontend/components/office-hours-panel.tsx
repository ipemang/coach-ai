...
export function OfficeHoursPanel() {
  ...
  function toggleDay(day: Day) {
    setState((s) => ({
      ...s,
      [day]: s[day] ? undefined : ["09:00", "18:00"],
    }));
  }
  ...
  async function save() {
    ...
    try {
      const body = {
        ...
        ...Object.fromEntries(
          DAYS.filter((d) => state[d]).map((d) => [d, state[d]])
        ),
      };
        ...
        <div className="absolute right-0 top-10 z-50 w-80 rounded-2xl border border-line bg-[#0f1117] p-5 shadow-2xl">
          ...
          <div className="mb-4 flex items-center justify-between rounded-xl border border-line bg-white/5 px-3 py-2.5">
            ...
            <button
              ...
              className={`relative h-6 w-11 rounded-full transition-colors ${
                state.ai_autonomy_override ? "bg-sky-500" : "bg-white/10"
              }`}
            >
              <span
                className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${
                  state.ai_autonomy_override ? "translate-x-5" : "translate-x-0.5"
                }`}
              />
            </button>
          </div>
              ...
              return (
                <div key={day} className="flex items-center gap-2">
                  <button
                    ...
                    className={`w-10 shrink-0 rounded-md px-1 py-1 text-xs font-medium transition ${
                      active
                  ...
                  {active && hours ? (
                    <div className="flex flex-1 items-center gap-1.5">
                      <input
                        ...
                        className="flex-1 rounded-md border border-line bg-white/5 px-1.5 py-1 text-xs text-white"
                      />
          ...
          <div className="mt-4 flex items-center gap-2">
            <button
              onClick={save}
              disabled={saving}
              className="flex-1 rounded-xl bg-indigo-500/20 py-2 text-sm font-medium text-indigo-300 transition hover:bg-indigo-500/30 disabled:opacity-40"
            >
