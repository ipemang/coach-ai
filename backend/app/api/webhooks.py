...
async def _get_ai_reply(request: Request, coach: dict[str, Any], athlete: AthleteRecord, text: str) -> str:
    # Use existing checkin service or fallback to simple AI mock
...
async def whatsapp_webhook_handshake(
    ...
    if hub_verify_token == get_settings().whatsapp_verify_token: return hub_challenge or ""
...
async def whatsapp_webhook(request: Request) -> WhatsAppWebhookResponse:
    ...
    # Simple extraction logic for phone and text
    sender = None
    text = None
    if "entry" in payload:
    ...
    # Purpose: coach→WhatsApp→AI→athlete
    # For now, we'll find the first athlete for this coach to context-load the AI
    ...
    # Send reply back to coach via WhatsApp
    whatsapp = request.app.state.whatsapp_service
    recipient = WhatsAppRecipient(athlete_id=athlete.athlete_id, phone_number=sender, display_name=coach.get("full_name"))
